...
```mermaid
flowchart TD
  A[Receive error] --> B{retriable?}
  B -->|Yes| C[Retry with backoff]
  B -->|No| D[Show user-facing failure]
  C --> E{still failing?}
  E -->|Yes| F[Escalate + logs]
  E -->|No| G[Continue]
```
...
function describe(code: NexisErrorCode): string {
  switch (code) {
    case 'auth.invalid_token':
      return 'Token is invalid or expired';
    case 'auth.project_mismatch':
      return 'Token does not match project';
    case 'room.not_found':
      return 'Requested room does not exist';
    case 'rpc.unknown_rid':
      return 'Unknown RPC correlation id';
    case 'state.seq_gap':
      return 'State sequence gap detected';
  }
}

// ---cut-before---
console.log(describe('rpc.unknown_rid'));
```
