...
If this fails, check [Connect a Client](/docs/getting-started/connect-client/) and [Auth in Real Games](/docs/getting-started/authentication/).

For dashboard usage details, see [Dashboard](/docs/infrastructure/dashboard/).

## Typed Quickstart Client

```ts twoslash
type Room = { send(type: string, payload: unknown): void };
type Client = { joinOrCreate(roomType: string, options: { room: string }): Promise<Room> };
declare function connect(url: string, options: { projectId: string; token: string }): Promise<Client>;
...
async function quickstart() {
  ...
  const room = await client.joinOrCreate('counter_plugin_room', { room: 'counter_plugin_room:default' });
  room.send('inc', { by: 1 });
}
```
