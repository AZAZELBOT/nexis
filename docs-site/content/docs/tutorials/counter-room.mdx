...
For protocol details see [Protocol](/docs/api-reference/protocol/).
...
```mermaid
flowchart LR
  A[Client sends inc] --> B[Room logic increments state]
  B --> C[state.patch emitted]
  C --> D[Client renders new counter]
```
...
```ts twoslash
type CounterAction = { by: number };
type CounterState = { counter: number };

function reduce(state: CounterState, action: CounterAction): CounterState {
  return { counter: state.counter + action.by };
}

// ---cut-before---
const next = reduce({ counter: 3 }, { by: 1 });
